...
double process_array(int threads, int* array, int count, int const target, int* index)
{
    *index = -1;
    ...
    #pragma omp parallel num_threads(threads) shared(array, count, target, index) 
    {
        #pragma omp for schedule(guided, CHUCK_SIZE) //schedule(static | dynamic | guided, chunk)
        for(int i=0; i<count; i++)
        {
            if(*index != -1)
                i = count;
